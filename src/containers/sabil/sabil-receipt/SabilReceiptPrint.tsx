...
const SabilReceiptPrint = () => {
  ...
  const [data, setData] = useState<ReceiptApiRow | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const payload = response?.data as { rows?: ReceiptApiRow[] } | undefined;
        if (payload?.rows?.[0]) {
          setData(payload.rows[0]);
        } else {
          setError(true);
        }
      } catch {
...
export default SabilReceiptPrint;
