...
import { Datagrid, TextField, DateField, FunctionField, useListContext } from "react-admin";
...
const RecipientsListContent = () => {
  ...
  if (error) {
    return <Typography color="error">Failed to load recipients: {error.message}</Typography>;
  }
  ...
  return (
    <Datagrid bulkActionButtons={false}>
      ...
      <FunctionField
        label="Status"
        render={(recipient) => <RecipientStatusChip status={recipient.status} />}
      />
