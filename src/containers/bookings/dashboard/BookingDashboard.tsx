...
type BookingStatsSummary = {
  paidAmount?: number;
  depositPaidAmount?: number;
  refundReturnAmount?: number;
  writeOffAmount?: number;
  extraExpenses?: number;
  jamaatLagat?: number;
};

type BookingStatsJson = {
  hallStats?: unknown[];
...
export default function BookingDashboard() {
  ...
  const [halls, setHalls] = useState<HallStatRow[]>([]);
  const [summary, setSummary] = useState<BookingStatsSummary | null>(null);
  const [totalRent, setTotalRent] = useState(0);
  const [error, setError] = useState<string | null>(null);

  const fetchBookingStats = async (sDate: string, eDate: string) => {
    setLoading(true);
    setError(null);
    try {
      const payload = { startDate: sDate, endDate: eDate };
      const resp = await callApi({ location: "bookingStats", data: payload, method: "GET" });
      const json = (resp.data as BookingStatsJson) || {};

      const hallsCandidate = (json.hallStats || []) as Record<string, unknown>[];
      const summaryCandidate = (json.bookingStats || {}) as Record<string, unknown>;
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (urlStartDate && urlEndDate) {
        ...
        void fetchBookingStats(urlStartDate, urlEndDate);
        return;
      }
    }

    void fetchBookingStats(defaultStartDate, defaultEndDate);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- initial load / URL bootstrap only
  }, []);

  const handleDateRangeChange = (newStartDate: string, newEndDate: string) => {
    setStartDate(newStartDate);
    setEndDate(newEndDate);
    void fetchBookingStats(newStartDate, newEndDate);
  };
  ...
  const totalBookings = halls.reduce((sum, h) => sum + (h.hallCount ?? 0), 0);
  const avgOccupancy =
    halls.length > 0
      ? Math.round(halls.reduce((sum, h) => sum + (h.occupancyPercentage ?? 0), 0) / halls.length)
      : 0;
  ...
  const bookingBreakdown = summary
    ? [
        ...
        { name: "Write Off", value: summary.writeOffAmount },
