...
const HallChargesTable = () => {
  ...
  const hallCharges = (record?.hallCharges as RaRecord[] | undefined) || [];
      ...
      <Table size="small">
        ...
        <TableBody>
          {hallCharges.map((charge, index) => (
            <TableRow key={(charge.hallId as string) || String(index)}>
              <TableCell>
                {(charge.hallName as string) ||
                  (charge.hallShortCode as string) ||
                  String(charge.hallId)}
              </TableCell>
              <TableCell align="right">
                {charge.rent ? `₹${Number(charge.rent).toLocaleString()}` : "-"}
              </TableCell>
              <TableCell align="right">
                {charge.deposit ? `₹${Number(charge.deposit).toLocaleString()}` : "-"}
              </TableCell>
              <TableCell align="right">
                {charge.acCharges ? `₹${Number(charge.acCharges).toLocaleString()}` : "-"}
              </TableCell>
              <TableCell align="right">
                {charge.kitchenCleaning
                  ? `₹${Number(charge.kitchenCleaning).toLocaleString()}`
                  : "-"}
              </TableCell>
