...
function typeDisplayName(type) {
  const m = { Jaman: "Jaman", Food_packets: "Food packets", Salawaat: "Salawaat" };
  return m[type] || type;
}

const LinkCard = ({ title, description, to, redirect }) => (
  <Card sx={{ minHeight: 112, width: "100%" }}>
    <CardActionArea onClick={() => redirect(to)} sx={{ height: "100%" }}>
      <CardContent>
...
export default function OhbatDashboard() {
  ...
  const canAny = (list) => hasAnyPermission(permissions, list);
  ...
  return (
    <>
      ...
      <Grid container spacing={2}>
        {linkCards.map((c) => (
          <Grid key={c.to} item size={{ xs: 12, sm: 6, md: 4 }}>
            <LinkCard {...c} redirect={redirect} />
          </Grid>
        ))}
      </Grid>
    </>
