...
function formatMobileForMessage(mobile) {
...
function formatTimeForMessage(hhmm) {
  const label = formatMajlisStartTimeLabel(hhmm);
  return label === "—" ? "" : label;
}

/** WhatsApp-style confirmation message for hosts. */
export function buildOhbatMajlisEventDetailsText(record) {
  if (!record) return "";
  ...
  const sadaratName = clip(record.sadarat?.name);
  const sadaratPhone = formatMobileForMessage(record.sadarat?.mobile);
