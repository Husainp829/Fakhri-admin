...
type LedgerReport = {
  ...
  creditTotalOnline?: number;
  debitTotalCash?: number;
  debitTotalOnline?: number;
  openingBalanceCash?: number;
  openingBalanceOnline?: number;
  closingBalanceCash?: number;
  closingBalanceOnline?: number;
  entryCount?: number;
};
...
const MiqaatNiyaazDashboard = () => {
  ...
  const [reportData, setReportData] = useState<LedgerReport | null>(null);
  ...
  const fetchLedgerReport = async (paymentMode: string) => {
    setLoading(true);
    try {
  ...
  const getFinancialYearDateRange = (fy: string) => {
  ...
  const summaryCardsConfig: {
    key: string;
    label: string;
    getValue: (data: LedgerReport, mode: string) => number | undefined;
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {summaryCardsConfig.map((card) => {
              ...
              const color = card.colorCondition(value ?? 0)
                ? card.colorPositive
                : card.colorNegative;
