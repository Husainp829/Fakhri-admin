...
import type { ReceiptGroupedMap } from "@/containers/events/types";

const calcRowTotal = (row: { CASH?: number; CHEQUE?: number; ONLINE?: number }) => {
...
type ModeTotals = { CASH?: number; CHEQUE?: number; ONLINE?: number };

const TotalRow = ({ values }: { values: Record<string, ModeTotals> }) => {
  const { CASH, CHEQUE, ONLINE } = Object.values(values).reduce<ModeTotals>(
    (acc, value) => {
      const { CASH: cash, CHEQUE: cheque, ONLINE: online } = value;
      acc.CASH = (acc.CASH ?? 0) + Number(cash || 0);
      acc.CHEQUE = (acc.CHEQUE ?? 0) + Number(cheque || 0);
      acc.ONLINE = (acc.ONLINE ?? 0) + Number(online || 0);
      return acc;
    },
...
type EventDashboardReceiptDayWiseProps = {
  receiptMap: ReceiptGroupedMap;
  selectedMarkaz: string;
};

export const EventDashboardReceiptDayWise = ({
  receiptMap,
  selectedMarkaz,
}: EventDashboardReceiptDayWiseProps) => {
  if (!selectedMarkaz) {
    return null;
  }
  const markazRows = receiptMap[selectedMarkaz] ?? {};
  return (
    <Grid
      sx={{ px: 1 }}
      size={{
    ...
    >
      <TableContainer component={Paper} sx={{ mb: 5 }}>
        <Table sx={{ minWidth: 100 }} aria-label="simple table">
          ...
          <TableBody>
            {Object.entries(markazRows).map(([key, value]) => (
