...
type FilterGroupProps = {
  ...
  onUpdate: (group: FilterGroupData) => void;
  onDelete: () => void;
  level?: number;
  availableColumns: ItsColumnDef[];
};
...
const FilterGroup = memo(
  ({
    ...
    const updateRule = useCallback(
      (ruleId: string, updatedRule: FilterRuleRow) => {
        onUpdate({
          ...group,
    ...
    const updateNestedGroup = useCallback(
      (groupId: string, updatedGroup: FilterGroupData) => {
        onUpdate({
          ...group,
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ py: 0.5, width: 60, textAlign: "center" }}></TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
          ...
          <Box sx={{ mb: 1 }}>
            {group.groups.map((nestedGroup) => (
              <Box key={nestedGroup.id} sx={{ ml: level > 0 ? 1 : 0, mt: 1 }}>
                <FilterGroup
                  group={nestedGroup}
                  onUpdate={(updated) => updateNestedGroup(nestedGroup.id, updated)}
