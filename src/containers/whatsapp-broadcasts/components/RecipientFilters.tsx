...
const CsvUploadTab = () => {
  ...
  const parsedCsvRef = useRef<ReturnType<typeof parseBroadcastCsv> | null>(null);
  ...
  useEffect(() => {
    ...
    if (matched.length > 0) {
      ...
      setValue("recipientItsIds", matched.map((r) => r.ITS_ID).filter(Boolean) as string[], {
        shouldDirty: false,
      });

      const parsed = parsedCsvRef.current;
      if (parsed && parsed.itsHeader && parsed.rows.length) {
        const rowsByIts: Record<string, Record<string, string>> = {};
        ...
        const csvDataByItsId: Record<string, Record<string, string>> = {};
        matched.forEach((r) => {
            ...
            <Card variant="outlined">
              <TableContainer sx={{ maxHeight: 500 }}>
                <Table size="small" stickyHeader>
                  ...
                  <TableBody>
                    {paginatedMatched.map((r: RaRecord) => (
                      <TableRow key={String(r.id ?? r.ITS_ID)}>
...
const RecipientFilters = () => {
  ...
  const handleTabChange = useCallback(
    (_: SyntheticEvent, newTab: number) => {
      setTab(newTab);
      updateSelectedPhones([]);
      updatePreviewStatus(false);
      setValue("recipientItsIds", null, { shouldDirty: false });
